...
def run_modbus_server(port, baudrate, bytesize, parity, stopbits, slave_id, 
    ...
    async def start_server():
        ...
        # LOG DETALHADO - Descomente para debug
        # print(f"\n{'='*80}")
        # print(f"\n📋 COILS (Tipo Modbus: Coil):")
        # for addr in sorted(coils_perm.keys()):
        #     fcs = "1/5/15" if 'W' in coils_perm[addr].upper() else "1"
        #     print(f"  Addr {addr}: Permissão={coils_perm[addr]}, FCs Modbus=[{fcs}]")
        # 
        # print(f"\n📋 DISCRETE INPUTS (Tipo Modbus: Discrete Input):")
        # for addr in sorted(di_perm.keys()):
        #     print(f"  Addr {addr}: Permissão={di_perm[addr]}, FCs Modbus=[2]")
        # 
        # print(f"\n📋 INPUT REGISTERS (Tipo Modbus: Input Register):")
        # for addr in sorted(ir_perm.keys()):
        #     print(f"  Addr {addr}: Permissão={ir_perm[addr]}, FCs Modbus=[4]")
        # 
        # print(f"\n📋 HOLDING REGISTERS (Tipo Modbus: Holding Register):")
        # for addr in sorted(hr_perm.keys()):
        #     fcs = "3/6/16" if 'W' in hr_perm[addr].upper() else "3"
        #     print(f"  Addr {addr}: Permissão={hr_perm[addr]}, FCs Modbus=[{fcs}]")
        # 
        # print(f"\n{'='*80}\n")
        ...
        # Loop infinito - será morto pelo kill()
