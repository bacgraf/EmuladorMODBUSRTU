...
class ServerThread(threading.Thread):
    """Thread para executar o servidor Modbus com tratamento de erros"""
    def __init__(self, server, error_callback):
    ...
    def run(self):
        try:
            ...
            print(f"❌ Erro na thread do servidor: {error_msg}")
            ...
            try:
                ...
                print(f"❌ Erro ao chamar callback de erro: {cb_error}")

    def stop(self):
        """Interromper a thread do servidor de forma segura"""
        self.running = False
        try:
            if self.server and hasattr(self.server, 'server_close'):
...
class ModbusEmulator(QMainWindow):
    ...
    def start_server(self):
        ...
        try:
            ...
            # Usar ServerThread para capturar erros durante execução
            self.server_thread = ServerThread(self.server, self.on_server_error_callback)
    ...
    def stop_server(self):
        ...
        try:
            ...
            # Aguardar parada da thread do servidor
            if self.server_thread and hasattr(self.server_thread, 'stop'):
                self.server_thread.stop()
                self.server_thread.join(timeout=2)  # Aguardar até 2 segundos
        except Exception as e:
