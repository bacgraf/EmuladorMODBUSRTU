"""Editor CSV para Mapa de Memória BMS - PyQt6"""
...
from PyQt6.QtGui import QColor, QIntValidator, QDoubleValidator, QIcon, QAction
import csv
import sys
...
class CSVEditor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("📝 Editor CSV - Mapa de Memória BMS")
        self.csv_path = ""
        self.modified = False
    ...
    def open_file(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(self, "Abrir CSV", "", "CSV files (*.csv);;All files (*.*)")
        if filename:
    ...
    def load_csv(self, filename):
        try:
            ...
            for idx, row_data in enumerate(data_rows):
                ...
                perm_combo.addItems(["R", "W", "RW"])
                perm_combo.setCurrentText(cols_data[6] if cols_data[6] in ["R", "W", "RW"] else "R")
    ...
    def save_file_as(self):
        filename, _ = QFileDialog.getSaveFileName(self, "Salvar CSV", "", "CSV files (*.csv);;All files (*.*)")
        if filename:
    ...
    def duplicate_row(self):
        ...
        if current_row >= 0:
            ...
            new_combo.setCurrentText(old_combo.currentText())
            ...
            # Copiar campos 1-5
            for col in range(1, 6):
                if self.table.item(current_row, col):
                    self.table.setItem(new_row, col, QTableWidgetItem(self.table.item(current_row, col).text()))
            
            # Copiar Permissao (col 6)
            old_perm_combo = self.table.cellWidget(current_row, 6)
            if old_perm_combo:
                new_perm_combo = NoWheelComboBox()
                new_perm_combo.addItems(["R", "W", "RW"])
                new_perm_combo.setCurrentText(old_perm_combo.currentText())
            ...
            if tipo in ["COIL", "DISC"]:
                ...
                if old_valor_combo:
                    ...
                    new_valor_combo.setCurrentText(old_valor_combo.currentText())
                    new_valor_combo.currentTextChanged.connect(lambda: self.mark_modified())
                    self.table.setCellWidget(new_row, 9, new_valor_combo)
            else:
                if self.table.item(current_row, 9):
                    self.table.setItem(new_row, 9, QTableWidgetItem(self.table.item(current_row, 9).text()))
            
            # Copiar Descricao (col 10)
            if self.table.item(current_row, 10):
                self.table.setItem(new_row, 10, QTableWidgetItem(self.table.item(current_row, 10).text()))
    ...
    def reload_table(self):
        ...
        for row_data in data:
            ...
            if self.dynamic_mode:
                ...
                perm_combo = NoWheelComboBox()
                perm_combo.addItems(["R", "W", "RW"])
                perm_combo.setCurrentText(row_data['col6'] if row_data['col6'] in ["R", "W", "RW"] else "R")
                perm_combo.currentTextChanged.connect(lambda p, r=row: self.on_permissao_changed(r, p))
                self.table.setCellWidget(row, 6, perm_combo)
